        ...
        HStack(spacing: 0) {
            ...
            Button {
                ...
                Text("Sana Özel")
                    ...
                    .foregroundStyle(selectedMode == .forYou ? .black : .white.opacity(0.6))
                    .fixedSize()
            ...
            Button {
                ...
                Text("Çifte Randevu")
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(selectedMode == .doubleDate ? .black : .white.opacity(0.6))
                    .fixedSize()
                    .padding(.horizontal, 14)
                    .padding(.vertical, 8)
                    .background {
...
struct GlassActionButton: View {
    ...
    var body: some View {
        Button {
            ...
            ZStack {
                // Daire - Normal: Koyu gri, Basılı: Gradient
                Circle()
                    .fill(
                        isActive ?
...
// MARK: - Likes View (Tinder Style - Who Liked You)
            ...
            ZStack(alignment: .bottom) {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        ...
                        // Tab Selector
                        HStack(spacing: 0) {
                            LikesTabButton(title: "\(likedYouUsers.count) Beğeni", isSelected: selectedTab == 0) {
                                withAnimation { selectedTab = 0 }
                            }
                            
                            LikesTabButton(title: "Süper Beğeni", isSelected: selectedTab == 1) {
                                withAnimation { selectedTab = 1 }
                            }
                        }
                        .padding(.horizontal, 20)
                        ...
                        // Features Section (only if not premium)
                        if !appState.isPremium {
                            VStack(spacing: 16) {
                                ...
                                HStack(spacing: 20) {
                                    FeatureItem(icon: "eye.fill", text: "Seni görüntüleyenleri gör")
                                    FeatureItem(icon: "heart.fill", text: "Sınırsız beğeni")
                                    FeatureItem(icon: "bolt.fill", text: "Öne çık")
                                }
                            }
                            .padding(.vertical, 20)
                            .padding(.horizontal, 16)
                ...
                // Floating Bottom Button (only if not premium)
                if !appState.isPremium {
                    Button {
                        showPremiumSheet = true
                    } label: {
...
// MARK: - Likes Tab Button
struct LikesTabButton: View {
    let title: String
    let isSelected: Bool
    let action: () -> Void
...
struct LikesCardView: View {
    ...
    var body: some View {
        Button(action: onTap) {
            GeometryReader { geo in
                ZStack(alignment: .bottom) {
                    // Photo with heavy blur when not premium
                    AsyncImage(url: URL(string: user.profilePhotoURL)) { phase in
                        switch phase {
                        case .success(let image):
                            image
                    ...
                    // Dark overlay for blurred state
                    if isBlurred {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        HStack(spacing: 4) {
                            Image(systemName: "location.fill")
                                .font(.system(size: 11))
                            Text("\(String(format: "%.1f", user.distanceKm ?? 0)) km uzakta")
                                .font(.system(size: 12))
                        }
                        .foregroundStyle(.white.opacity(0.7))
